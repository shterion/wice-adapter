...
function processTrigger(msg, cfg, snapshot = {}) {
  ...
  async function getCategoryRowid(cookie) {
    try {
      const uri = `https://oihwice.wice-net.de/plugin/wp_wice_client_api_backend/json?method=get_all_companies&cookie=${cookie}`;
  ...
  async function getDeletedOrganizations() {
    try {
      ...
      const uri = `https://oihwice.wice-net.de/plugin/wp_wice_client_api_backend/json?method=get_all_companies&full_list=1&address_category1=${categoryId}&select_category1=${categoryId}&ext_search_do=1&cookie=${cookie}`;
      const options = {
        uri,
        headers: {
