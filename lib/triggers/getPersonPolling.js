...
function processTrigger(msg, cfg) {
  ...
  wice.createSession(cfg, () => {
      ...
      let uri = `https://oihwice.wice-net.de/plugin/wp_elasticio_backend/json?method=get_all_persons&cookie=${cfg.cookie}&pkey=${rowid}`;
      let requestOptions = {
        uri,
        headers: {
      ...
      // Make a get request to get all persons saved by a specific user in wice
      request.get(requestOptions).then((res) => {

        let resObj = JSON.parse(res);
        contact = resObj.loop_addresses;
        emitData();
