...
function processAction(msg, cfg) {
  ...
  wice.createSession(cfg, () => {
    if (cfg.cookie) {
      ...
      let uri = `https://oihwice.wice-net.de/plugin/wp_elasticio_backend/json?method=get_all_persons&cookie=${cfg.cookie}&name=${contact.name}`;
      let existingRowid = 0;
      let requestOptions = {
        uri,
        headers: {
      ...
      checkForExistingUser().then((existingRowid) => {
        if (existingRowid == 0) {
      ...
      function checkForExistingUser() {
        return new Promise((resolve, reject) => {
          request.get(requestOptions).then((res) => {
            ...
            if (resObj.loop_addresses.length > 0) {
