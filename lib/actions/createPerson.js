  ...
  wice.createSession(cfg, () => {
    if (cfg.cookie) {
      ...
      for (var i = 0; i < msg.body.length; i++) {
        console.log(JSON.stringify(msg.body[i]));
      }

      // for (var person in msg.body) {
      //
      //   console.log(msg.body[person]);
      //
      //     checkForExistingUser().then(() => {
      //       if (existingRowid == 0) {
      //         console.log('Creating a person ...');
      //         requestToWice('insert_contact', JSON.stringify(person));
      //
      //       } else {
      //         msg.body.rowid = existingRowid;
      //         requestToWice('update_contact', JSON.stringify(person));
      //       }
      //     });
      // }
