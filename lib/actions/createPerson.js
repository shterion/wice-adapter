...
function processAction(msg, cfg) {
  ...
  let json = JSON.stringify(msg, undefined, 2);
  console.log(`MSG: ${json.length}`);
  ...
  let getLengthMsg = Object.keys(msg).length;
  let getLengthBody = Object.keys(body).length;

  // console.log(`MSG LENGTH: ${getLengthMsg}`);
  // console.log('END');
  // console.log(`MSG BODY LENGTH: ${getLengthBody}`);
  ...
  wice.createSession(cfg, () => {
    if (cfg.cookie) {
      ...
      //   let user = JSON.stringify(body[i]);
      //   console.log('FLAG 1');
      //
      ...
      function checkForExistingUser() {
        options.form = {
          ...
          cookie: cfg.cookie,
          ext_search_do: 1,
          name: body.name // Best practice is to use email but at this point of time the email is not accesible
        };
